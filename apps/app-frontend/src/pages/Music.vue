<template>
	<div class="music-page box-border min-h-full p-4">
		...
		<p class="m-0 mb-6 text-secondary">
			ВНИМАНИЕ ЕСЛИ ВЫ ПО КАКОЙ ТО ПРИЧИНЕ УВИДЕЛИ ЭТУ ВКЛАДКУ ОТКЛЮЧИТЕ ЕЕ В НАСТРОЙКАХ
			И СДЕЛАЙТЕ ВИД ЧТО ЕЕ НЕ ВИДЕЛИ, ПРИ ПОПЫТКЕ ЧТО ТО ТУТ ИЗМЕНИТЬ ВЫ МОЖЕТЕ СЛОМАТЬ ИГРУ И САМ ЛАУНЧЕР
		...
		<div class="settings-section">
			...
			<p class="settings-section__desc settings-section__warning">
				...
				open-source инструментом на своей машине) и вставь его ниже. Токен хранится только
				локально на этом устройстве и отправляется только на api.vk.com — никогда на сервера
				ModLEX (их и нет). Доступ к аудио VK через сторонний токен нарушает правила VK и может
				привести к блокировке твоего аккаунта. Используешь на свой риск.
			</p>
			<div class="setting-row">
				<div class="setting-row__info">
		...
		<div class="settings-section">
			...
			<p class="settings-section__desc">
				Собери плейлисты из локальных файлов и прямых радио-ссылок — мод подхватит их как
				стартовые очереди. Добавление из SoundCloud и VK появится, когда будут готовы резолверы
				на стороне мода.
			</p>
					...
					>
						<template #item="{ element: track, index: trackIndex }">
							<div class="playlist-track">
								...
								<button type="button" class="playlist-track__remove" @click="removeTrack(trackIndex)">
									<XIcon />
								</button>
							</div>
						</template>
