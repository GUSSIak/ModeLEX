<script setup lang="ts">
	...
	count: 0,
	icon: null,
	checked: selectedCategory.value === cat.id,
	negative: false,
	})),
	},
	{
	...
	count: 0,
	icon: null,
	checked: selectedGameVersion.value === version,
	negative: false,
	})),
	},
	{
	...
	return {
	id: loaderName,
	name: loaderName.charAt(0).toUpperCase() + loaderName.slice(1),
	count: 0,
	icon: null,
	checked: selectedLoader.value === loader,
	negative: false,
	}
	}),
	},
	...
	// ===== PROVIDED FILTERS (для блокировки) =====
	...
	// ===== TOGGLED GROUPS =====
	const toggledGroups = ref<Record<string, boolean>>({})
	...
	gameVersions.value = gvs?.map(g => g.version).filter(v => v) || []
	loaders.value = lds?.map(l => l.loader).filter(v => v) || []
	} catch (e) {
	handleError(e)
	}
	}
	...
	onMounted(() => {
	loadData()
	// Проверяем наличие инстанса в URL
	...
	return 'border-0 border-b-[1px] [&:first-child>button]:pt-4 last:border-b-0 border-[--cf-gradient-border] border-solid'
	})
	...
	return filterTypes.value.filter(f => f.options.length > 0)
	})
</script>
	...
	<div class="cf-sidebar-wrapper">
        ...
        'fixed inset-0 z-50 m-4 mb-0 overflow-auto rounded-t-3xl bg-bg-raised': filtersMenuOpen,
			...
			<template v-for="filterType in filterOptions" :key="`cf-filter-${filterType.id}`">
				<SearchSidebarFilter v-model:selected-filters="currentFilters"
					...
					<template v-if="lockedMessages?.gameVersion && filterType.id === 'game_version'" #locked-game_version>
						{{ lockedMessages.gameVersion }}
					</template>
					<template v-if="lockedMessages?.modLoader && filterType.id === 'mod_loader'" #locked-mod_loader>
